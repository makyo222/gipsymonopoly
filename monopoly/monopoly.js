function Game() {
	...
	this.next = function() {
		if (!p.human && p.money < 0) {
			...
			if (p.money < 0) {
				popup("<p>" + p.name + " csődbe ment. Mindenét átadja " + player[p.creditor].name + ".</p>", game.bankruptcy);
			} else {
				roll();
			}
	...
	var finalizeAuction = function() {
		...
		if (highestbid > 0) {
			...
			addAlert(p.name + " vesz " + sq.name + " ennyiért $" + highestbid + ".");
		}
	...
	this.auction = function() {
		...
		popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Aukció <span id='propertyname'></span></div><div>Legmagasabb bid = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, te jössz hogy bideljél.</div<div><input id='bid' title='Mennyiért bidelsz " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='Bid' onclick='game.auctionBid();' title='Írd be a bidet.' /><input type='button' value='Passz' title='Skippelem a bidet.' onclick='game.auctionPass();' /><input type='button' value='Kilépés az aukcióból' title='Stop bidelés on " + s.name + " altogether.' onclick='if (confirm(\"Biztos?\")) game.auctionExit();' /></div>", "blank");
			...
			if (currentbidder == highestbidder) {
				...
				if (!p.human) {
					...
					if (bid === -1 || highestbid >= p.money) {
						...
						window.alert(p.name + " kilép az aukcióból");
						continue;

					} else if (bid === 0) {
						window.alert(p.name + " passzol.");
						continue;

					} else if (bid > 0) {
						this.auctionBid(bid);
						window.alert(p.name + " bidel $" + bid + ".");
						continue;
					}
					return;
				} else {
	...
	this.auctionBid = function(bid) {
		...
		if (bid === "" || bid === null) {
			document.getElementById("bid").value = "Add meg a bidet.";
			document.getElementById("bid").style.color = "red";
		} else if (isNaN(bid)) {
			document.getElementById("bid").value = "Szám legyen.";
			document.getElementById("bid").style.color = "red";
		} else {

			if (bid > player[currentbidder].money) {
				document.getElementById("bid").value = "Nincs elég pénzed $" + bid + ".";
				document.getElementById("bid").style.color = "red";
				...
				document.getElementById("bid").value = "Nagyobbat bideljél ($" + highestbid + ")";
				document.getElementById("bid").style.color = "red";
			}
		}
	};
	...
	var tradeMoneyOnChange = function(e) {
		...
		if (amount < 0) {
			this.value = "Nagyobb legyen mint 0.";
			this.style.color = "red";
			return false;
		}
	...
	var resetTrade = function(initiator, recipient, allowRecipientToBeChanged) {
		...
		for (var i = 0; i < 40; i++) {
			...
			if (currentSquare.owner === initiator.index) {
				...
				currentTableCellCheckbox.title = "Jelöld be hogy " + currentSquare.name + " trédeljen.";
				...
				if (currentSquare.mortgage) {
					currentTableCell.title = "Jelzáloggal terhelt";
		...
		if (initiator.communityChestJailCard) {
			...
			currentTableCellCheckbox.title = "Jelöld be a szökéshez";
			...
			currentTableCell.textContent = "Szökés kártya";
		...
		if (initiatorSideTable.lastChild) {
			...
			initiatorProperty.textContent = initiator.name + " nincs mit trédelni.";
		}

		if (recipientSideTable.lastChild) {
			recipientProperty.appendChild(recipientSideTable);
		} else {
			recipientProperty.textContent = recipient.name + " nincs mit trédelni.";
		}
		...
		if (allowRecipientToBeChanged && pcount > 2) {
			...
			nameSelect.title = "Kivel trédelsz";
		} else {
	...
	this.acceptTrade = function(tradeObj) {
		if (isNaN(document.getElementById("trade-leftp-money").value)) {
			document.getElementById("trade-leftp-money").value = "Szám legyen";
			document.getElementById("trade-leftp-money").style.color = "red";
			return false;
		}

		if (isNaN(document.getElementById("trade-rightp-money").value)) {
			document.getElementById("trade-rightp-money").value = "Szám legyen";
		...
		if (money > 0 && money > initiator.money) {
			document.getElementById("trade-leftp-money").value = initiator.name + " nincs $" + money + ".";
			document.getElementById("trade-leftp-money").style.color = "red";
			return false;
		} else if (money < 0 && -money > recipient.money) {
			document.getElementById("trade-rightp-money").value = recipient.name + " nincs $" + (-money) + ".";
			document.getElementById("trade-rightp-money").style.color = "red";
			return false;
		}
		...
		if (isAPropertySelected === 0) {
			popup("<p>Válassz trédelést</p>");

			return false;
		}

		if (showAlerts && !confirm(initiator.name + ", biztos trédel " + recipient.name + "?")) {
			return false;
		}
		...
		for (var i = 0; i < 40; i++) {
			...
			if (tradeObj.getProperty(i) === 1) {
				...
				addAlert(recipient.name + " megjött " + square[i].name + " tőle " + initiator.name + ".");
			} else if (tradeObj.getProperty(i) === -1) {
				square[i].owner = initiator.index;
				addAlert(initiator.name + " megjött " + square[i].name + " tőle " + recipient.name + ".");
			}
		...
		if (tradeObj.getCommunityChestJailCard() === 1) {
			...
			addAlert(recipient.name + ' megjött a "Szökés kártya" tőle ' + initiator.name + ".");
		} else if (tradeObj.getCommunityChestJailCard() === -1) {
			initiator.communityChestJailCard = true;
			recipient.communityChestJailCard = false;
			addAlert(initiator.name + ' megjött a "Szökés kártya" tőle ' + recipient.name + ".");
		}
		...
		if (money > 0) {
			...
			addAlert(recipient.name + " megjött $" + money + " tőle " + initiator.name + ".");
		} else if (money < 0) {
			money = -money;
			...
			addAlert(initiator.name + " megjött $" + money + " tőle " + recipient.name + ".");
		}
	...
	this.proposeTrade = function() {
		...
		if (initiator.human && !confirm(initiator.name + ", biztos trédel " + recipient.name + "?")) {
			return false;
		}
		...
		if (recipient.human) {
			...
			addAlert(initiator.name + " trédet kezdeményez vele " + recipient.name + ".");
			popup("<p>" + initiator.name + " trédet javasol, " + recipient.name + ". Elfogad, elutasít, módosít.</p>");
		} else {
			var tradeResponse = recipient.AI.acceptTrade(tradeObj);

			if (tradeResponse === true) {
				popup("<p>" + recipient.name + " elfogadta.</p>");
				this.acceptTrade(reversedTrade);
			} else if (tradeResponse === false) {
				popup("<p>" + recipient.name + " elutasította.</p>");
				return;
			} else if (tradeResponse instanceof Trade) {
				popup("<p>" + recipient.name + " kontra ajánlatot tett.</p>");
	...
	this.eliminatePlayer = function() {
		...
		if (pcount === 1) {
			...
			popup("<p>Gratulálok, " + player[1].name + ", nyertél.</p><div>");
	...
	this.resign = function() {
		popup("<p>Biztos?</p>", game.bankruptcy, "Yes/No");
	};
	...
	this.bankruptcy = function() {
		...
		if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
			...
			addAlert(pcredit.name + " paid $" + bankruptcyUnmortgageFee + " interest on the mortgaged properties megjött tőle " + p.name + ".");
			popup("<p>" + pcredit.name + ", you must pay $" + bankruptcyUnmortgageFee + " interest on the mortgaged properties you megjött tőle " + p.name + ".</p>", function() {player[pcredit.index].pay(bankruptcyUnmortgageFee, 0); game.bankruptcyUnmortgage();});
		}
	};
...
function Trade(initiator, recipient, money, property, communityChestJailCard, chanceJailCard) {
	// For each property and Szökés kártyas, 1 means offered, -1 means requested, 0 means neither.
...
// Overwrite an array with numbers tőle one to the array's length in a random order.
...
function updateOwned() {
	...
	for (var i = 0; i < 40; i++) {
		...
		if (sq.owner == turn) {
			...
			mortgagetext = "";
			if (sq.mortgage) {
				mortgagetext = "title='Jelzáloggal terhelt' style='color: grey;'";
			}

			housetext = "";
			if (sq.house >= 1 && sq.house <= 4) {
				for (var x = 1; x <= sq.house; x++) {
					housetext += "<img src='images/house.png' alt='' title='Ház' class='house' />";
				}
			} else if (sq.hotel) {
	...
	if (p.communityChestJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox40' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Szökés kártya</td></tr>";
	...
	if (p.chanceJailCard) {
		...
		HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox41' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Szökés kártya</td></tr>";
	}

	if (HTML === "") {
		HTML = p.name + ", nincs ingatlanod.";
		$("#option").hide();
	} else {
		HTML += "</table>";
	}
...
function updateOption() {
	...
	if (sq.mortgage) {
		...
		if (sq.groupNumber >= 3) {
			...
			buyhousebutton.value = "Ház vétel ($" + sq.houseprice + ")";
			sellhousebutton.value = "Ház eladás ($" + (sq.houseprice * 0.5) + ")";
			buyhousebutton.title = "Ház vétel for $" + sq.houseprice;
			sellhousebutton.title = "Ház eladás for $" + (sq.houseprice * 0.5);

			if (sq.house == 4) {
				buyhousebutton.value = "Hotel vétel ($" + sq.houseprice + ")";
				buyhousebutton.title = "Hotel vétel for $" + sq.houseprice;
			}
			if (sq.hotel == 1) {
				$("#buyhousebutton").hide();
				sellhousebutton.value = "Hotel eladás ($" + (sq.houseprice * 0.5) + ")";
				sellhousebutton.title = "Hotel eladás for $" + (sq.houseprice * 0.5);
			}
			...
			for (var i = 0; i < max; i++) {
				...
				if (s.owner !== sq.owner) {
					...
					buyhousebutton.title = "Mielőtt házat vennél legyen meg az összes a színből";
				} else {
...
function chanceCommunityChest() {
	...
	if (p.position === 2 || p.position === 17 || p.position === 33) {
		...
		// Remove the Szökés kártya tőle the deck.
...
function addamount(amount, cause) {
	...
	addAlert(p.name + " megjött $" + amount + " tőle " + cause + ".");
}
...
function subtractamount(amount, cause) {
	...
	addAlert(p.name + " elvett $" + amount + " tőle " + cause + ".");
}

function gotojail() {
	var p = player[turn];
	addAlert(p.name + " börtönbe ment.");
	document.getElementById("landed").innerHTML = "Lecsuktak.";

	p.jail = true;
	doublecount = 0;

	document.getElementById("nextbutton").value = "Kör vége";
	document.getElementById("nextbutton").title = "Kör vége, kövi játékos.";
...
function payeachplayer(amount, cause) {
	...
	addAlert(p.name + " elvett $" + total + " tőle " + cause + ".");
}
...
function collectfromeachplayer(amount, cause) {
	...
	addAlert(p.name + " megjött $" + total + " tőle " + cause + ".");
}
...
function advance(destination, pass) {
	...
	if (typeof pass === "number") {
		if (p.position < pass) {
			...
			p.position = pass;
			p.money += 200;
			addAlert(p.name + " megkapta a $200.");
		}
	}
...
function payfifty() {
	...
	addAlert(p.name + " fizesd le az őrt $50.");
	updateMoney();
	updatePosition();
}
...
function useJailCard() {
	...
	if (p.communityChestJailCard) {
		...
		// Insert the Szökés kártya back into the community chest deck.
		...
		// Insert the Szökés kártya back into the chance deck.
	...
	addAlert(p.name + " használ  \"Szökés kártya.");
	updateOwned();
	updatePosition();
}
	...
	if (p.money - sq.houseprice < 0) {
		...
		if (sq.house < 4) {
			if (houseSum >= 32) {
				...
				sq.house++;
				addAlert(p.name + " ház építése " + sq.name + ".");
			}
			...
			if (hotelSum >= 12) {
				...
				sq.house = 5;
				sq.hotel = 1;
				addAlert(p.name + " hotel építése " + sq.name + ".");
			}
		}
...
function sellHouse(index) {
	...
	if (sq.hotel === 1) {
		sq.hotel = 0;
		sq.house = 4;
		addAlert(p.name + " hotel eladása " + sq.name + ".");
	} else {
		sq.house--;
		addAlert(p.name + " ház eladása " + sq.name + ".");
	}
...
function showStats() {
	...
	for (var x = 1; x <= pcount; x++) {
		...
		if (p.communityChestJailCard) {
			...
			HTML += "<tr><td class='statscellcolor'></td><td class='statscellname'>Szökés kártya</td></tr>";
...
function buy() {
	...
	if (p.money >= cost) {
		...
		property.owner = turn;
		updateMoney();
		addAlert(p.name + " vett " + property.name + " for " + property.pricetext + ".");
		...
		popup("<p>" + p.name + ", kell $" + (property.price - p.money) + " több hogy vegyél " + property.name + ".</p>");
	}
}
...
function land(increasedRent) {
	...
	$("#landed").show();
	document.getElementById("landed").innerHTML = "Ide kerültél: " + s.name + ".";
	s.landcount++;
	addAlert(p.name + " került ide: " + s.name + ".");
	...
	if (s.price !== 0 && s.owner === 0) {
		...
		if (!p.human) {
			...
			document.getElementById("landed").innerHTML = "<div>Ide került: <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Buy ($" + s.price + ")' title='Buy " + s.name + " for " + s.pricetext + ".'/></div>";
		}
	...
	if (s.owner !== 0 && s.owner != turn && !s.mortgage) {
		...
		addAlert(p.name + " fizess $" + rent + " bérleti díjat " + player[s.owner].name + ".");
		p.pay(rent, s.owner);
		player[s.owner].money += rent;

		document.getElementById("landed").innerHTML = "Ide került: " + s.name + ". " + player[s.owner].name + " collected $" + rent + " rent.";
	} else if (s.owner > 0 && s.owner != turn && s.mortgage) {
		document.getElementById("landed").innerHTML = "Ide került: " + s.name + ". Property is mortgaged; no rent was collected.";
	}
	...
	if (p.position === 30) {
		...
		if (p.human) {
			popup("<div>Börtönbe mész</div>", gotojail);
		} else {
			gotojail();
		}
...
function roll() {
	...
	if (die1 == die2) {
		addAlert(p.name + " dobott " + (die1 + die2) + " - dupla.");
	} else {
		addAlert(p.name + " dobott " + (die1 + die2) + ".");
	}
	...
	if (die1 == die2 && !p.jail) {
		...
		if (doublecount < 3) {
			document.getElementById("nextbutton").value = "Dobj újra";
			document.getElementById("nextbutton").title = "Duplát doptál. Dobj újra.";

		// If player rolls doubles three times in a row, send him to jail
		} else if (doublecount === 3) {
			p.jail = true;
			doublecount = 0;
			addAlert(p.name + " duplát dobott 3szor.");
			updateMoney();


			if (p.human) {
				popup("Rábasztál. Mész a böribe.", gotojail);
			} else {
				gotojail();
			}
	...
	if (p.jail === true) {
		...
		if (die1 == die2) {
			...
			addAlert(p.name + " duplát dob hogy kijön böriből.");
			...
			if (p.jailroll === 3) {
				...
				if (p.human) {
					popup("<p>Fizesd le az őröket $50.</p>", function() {
						payfifty();
...
function play() {
	...
	document.getElementById("nextbutton").value = "Dobj";
	document.getElementById("nextbutton").title = "Dobj és lépj.";
	...
	if (p.jail) {
		$("#landed").show();
		document.getElementById("landed").innerHTML = "Lecsuktak.<input type='button' title='Fizesd le az őröket.' value='Pay $50 fine' onclick='payfifty();' />";

		if (p.communityChestJailCard || p.chanceJailCard) {
			document.getElementById("landed").innerHTML += "<input type='button' id='gojfbutton' title='Use &quot;Szökés kártya' onclick=useJailCard(); value='Kártya használata' />";
		}

		document.getElementById("nextbutton").title = "Dobj. Ha dupla kiengednek.";

		if (p.jailroll === 0)
			addAlert("Ez " + p.name + "'s első kör a böriben.");
		else if (p.jailroll === 1)
			addAlert("Ez " + p.name + "'s második kör a böriben.");
		else if (p.jailroll === 2) {
			document.getElementById("landed").innerHTML += "<div>Ha nem tudsz duplát dobni ki <i>KELL</i> fizetned az $50 az őröknek.</div>";
			addAlert("Ez " + p.name + "'s harmadik kör a böriben.");
		}
...
window.onload = function() {
	...
	$("<span>").text("Börtön").appendTo("#jail");

	document.getElementById("jail").enlargeId = "enlarge40";

	document.getElementById("enlarge-wrap").innerHTML += "<div id='enlarge40' class='enlarge'><div id='enlarge40color' class='enlarge-color'></div><br /><div id='enlarge40name' class='enlarge-name'>Börtön</div><br /><div id='enlarge40price' class='enlarge-price'><img src='images/jake_icon.png' height='80' width='80' alt='' style='position: relative; top: -20px;' /></div><br /><div id='enlarge40token' class='enlarge-token'></div></div>";

	document.getElementById("enlarge40name").innerHTML = "Börtön";
	...
	$("#buyhousebutton").on("click", function() {
		...
		if (p.money < s.houseprice) {
			if (s.house === 4) {
				popup("<p>Kell $" + (s.houseprice - player[s.owner].money) + " több hogy vegyél hotel " + s.name + ".</p>");
				return;
			} else {
				popup("<p>Kell $" + (s.houseprice - player[s.owner].money) + " több hogy vegyél ház " + s.name + ".</p>");
				return;
			}
		}
		...
		if (s.house < 4 && houseSum >= 32) {
			popup("<p>Mind a 32 ház elkelt</p>");
			return;
		} else if (s.house === 4 && hotelSum >= 12) {
			popup("<p>Mind a 12 hotel elkelt.</p>");
			return;
		}
